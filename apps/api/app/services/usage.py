...
`source` says which of the two `conversation_id` is, since both are 32-hex uuids and
...
UsageSource = Literal["run", "assistant"]
...
async def record_usage(
    ...
    *,
    ...
    `source` disambiguates which one it is — "run" (the executor) or "assistant" (the
    builder assistant) — leave it `None` only for call sites that predate this field.
    """
